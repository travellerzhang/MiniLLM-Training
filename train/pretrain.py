...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, total_steps=None, warmup_steps=None, full_save_dir=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # 在with下面将数据转化成半精度
        with autocast_ctx:
            res = model(input_ids, labels=labels) # 模型的输出结果
            loss = res.loss / args.accumulation_steps # 计算损失
        scaler.scale(loss).backward() # 防止下溢出

        # 进行梯度累积，以时间换空间，然后进行梯度更新
...
if __name__ == "__main__":
    ...
    if args.use_swanlab:
        import swanlab
        swanlab.login(api_key="Cvh4R63bj1zZPYHVhpfdg")
    ...
    # 开始训练前先用benchmark评测一次，用于和最终结果进行比较
    ...
    optimizer = optim.AdamW(model.parameters(), lr=args.learning_rate, weight_decay=0.1) #防止过拟合
